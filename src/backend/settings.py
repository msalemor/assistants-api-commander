...
class Settings:
    def __init__(self):
        ...
        self.assistants_api_version = os.getenv("OPENAI_ASSISTANT_VERSION")
        self.chat_model = os.getenv("OPENAI_GPT_DEPLOYMENT")
...
def Instance() -> Settings:
    ...
    if settings is None:
        settings = Settings()
        if settings.api_endpoint is None or settings.api_key is None or settings.api_version is None or settings.chat_model is None or settings.email_URI is None:
            logging.error(
